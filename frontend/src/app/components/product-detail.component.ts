...
export class ProductDetailComponent implements OnInit {
  ...
  checkIfFavorited() {
    ...
    this.http.get<boolean>(`http://localhost:18081/api/v1/favoris/check?userId=${userId}&productId=${this.product.id}`)
  ...
  toggleFavorite() {
    ...
    const userId = this.authService.getUserId();
    this.http.post<boolean>(`http://localhost:18081/api/v1/favoris/toggle?userId=${userId}&productId=${this.product.id}`, {})
      .subscribe(res => {
